...
class CustomDataset(Dataset):
    ...
    def __getitem__(self, idx):
        img_name = os.path.join(
            self.image_folder, self.data_list[idx] + '.jpg')
        image = Image.open(img_name).convert('RGB')
        label_name = os.path.join(
            self.labels_folder, self.data_list[idx] + '.txt')
        label = []
...
model = ov.Core().read_model("./models/buff-1.xml")

# Provide validation part of the dataset to collect statistics needed for the compression algorithm
data_path = './datasets/buff_data'
val_dataset = CustomDataset(
    data_path, split='', transform=transforms.Compose([transforms.ToTensor()]))
